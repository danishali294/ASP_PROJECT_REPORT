...
num_trial = 10; % Number of independent realizations (to be averaged over)
...
avg_mseP = zeros(size(sim_res,2),size(sim_snr,2));
avg_mse0 = zeros(size(sim_res,2),size(sim_snr,2));
avg_mse1 = zeros(size(sim_res,2),size(sim_snr,2));
avg_mse2 = zeros(size(sim_res,2),size(sim_snr,2));
avg_mse3 = zeros(size(sim_res,2),size(sim_snr,2));
avg_mse4 = zeros(size(sim_res,2),size(sim_snr,2));
avg_mse5 = zeros(size(sim_res,2),size(sim_snr,2));
...
% DFT-MTC
ChoosensetT_4   = randsample(1:1:size(DFT_BS_INTEND,2),32);
ChoosensetR_4   = randsample(1:1:size(DFT_MS,2),16);
P4 = DFT_BS_INTEND(:,ChoosensetT_4);
Q4 = DFT_MS(:,ChoosensetR_4);

MEA4   = kron(transpose(P4),Q4');
MEA4O  = MEA4*kron(conj(AbG(:,Search_Range)),AmG);
...
avg_mseP(run_res,run_snr) = (MSE_SUMP/num_trial);
avg_mse0(run_res,run_snr) = (MSE_SUM0/num_trial);
avg_mse1(run_res,run_snr) = (MSE_SUM1/num_trial);
avg_mse2(run_res,run_snr) = (MSE_SUM2/num_trial);
avg_mse3(run_res,run_snr) = (MSE_SUM3/num_trial);
avg_mse4(run_res,run_snr) = (MSE_SUM4/num_trial);
avg_mse5(run_res,run_snr) = (MSE_SUM5/num_trial);
end
end
